...
+(void)dealForHotRouteWithDay:(NSString *)day WithSucess:(SucessBlock)sucessBlock FailureBlock:(FailureBlock)failureBlock
{
    ...
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:[hotRouteAndClassicsModel dictionaryForDay:day] options:0 error:NULL];
    [hotRouteAndClassicsModel dealWithResponseObject:jsonData WithModel:@"HotRouteModel"];
    sucessBlock(hotRouteAndClassicsModel.dataDictionary);
...
+(void)dealForClassicscWithSucess:(SucessBlock)sucessBlock FailureBlock:(FailureBlock)failureBlock
{
    ...
    [hotRouteAndClassicsModel dealWithResponseObject:jsonData WithModel:@"ClassicsModel"];
    sucessBlock(hotRouteAndClassicsModel.dataDictionary);
...
- (void)dealWithResponseObject:(NSData *)data WithModel:(NSString *)model
{
    ...
    if ([model isEqualToString:@"HotRouteModel"]) {
        [modelArray addObjectsFromArray:[HotRouteModel mj_objectArrayWithKeyValuesArray:arr[@"RouteList"]]];
    }
    if ([model isEqualToString:@"ClassicsModel"]) {
        [modelArray addObjectsFromArray:[ClassicsModel mj_objectArrayWithKeyValuesArray:arr[@"RouteList"]]];
    }
